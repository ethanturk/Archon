  ...
  const loadCredentials = async () => {
    try {
      ...
      const apiKeys = allCredentials.filter(cred => {
        ...
        return key.includes('_KEY') || key.includes('_API') || key.includes('API_') || key.includes('_BASE_URL') || cred.category === 'api_keys';
      });
      ...
      const uiCredentials = apiKeys.map(cred => {
        ...
        return {
          ...
          showValue: false,
          isNew: false,
          isFromBackend: true, // Mark as from backend unless it's a new credential
        };
      });
