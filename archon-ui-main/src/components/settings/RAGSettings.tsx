...
import { Check, Save, Loader, ChevronDown, ChevronUp, Zap, Database, Trash2, Cog } from 'lucide-react';
...
export const RAGSettings = ({
  ...
  const handleTestLLMConnection = async () => {
    ...
    try {
      // Auto-save settings before testing to ensure we test what's currently entered
      ...
      // Save logic (mirrors the Save Settings button logic but without toast if possible, or just standard save)
      ...
      // Determine service type based on active selection ('chat' -> 'llm', 'embedding' -> 'embedding')
      const serviceType = activeSelection === 'chat' ? 'llm' : 'embedding';
      const result = await credentialsService.testLLMConnection(serviceType);
  ...
  const getProviderStatus = (providerKey: string): 'configured' | 'missing' | 'partial' => {
    switch (providerKey) {
      case 'openai':
        return hasApiCredential('OPENAI_API_KEY') ? 'configured' : 'missing';
      case 'google':
        return hasApiCredential('GOOGLE_API_KEY') ? 'configured' : 'missing';
      case 'ollama':
        {
      ...
      case 'anthropic':
        return hasApiCredential('ANTHROPIC_API_KEY') ? 'configured' : 'missing';
      case 'grok':
        return hasApiCredential('GROK_API_KEY') ? 'configured' : 'missing';
      case 'openrouter':
        return hasApiCredential('OPENROUTER_API_KEY') ? 'configured' : 'missing';
      default:
        return 'missing';
    }
  };
  ...
  return <Card accentColor="green" className="overflow-hidden p-8">
    ...
    <div className="mb-6">
      ...
      <div className={`grid gap-3 mb-4 ${activeSelection === 'chat' ? 'grid-cols-6' : 'grid-cols-4'
        }`}>
        {[
      ...
      <div className="flex justify-between items-end">
        ...
        {/* Test Connection Button (General) */}
        <Button
          variant="outline"
          accentColor="blue"
          icon={testingLLMConnection ? <Loader className="w-4 h-4 mr-1 animate-spin" /> : <Zap className="w-4 h-4 mr-1 text-blue-500" />}
          className="whitespace-nowrap ml-4"
          size="md"
          onClick={handleTestLLMConnection}
          disabled={testingLLMConnection || saving}
          title="Save and test connection with currently entered settings"
        >
